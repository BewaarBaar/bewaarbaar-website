...
const BeeTimeline = () => {
  ...
  // Compute dot positions once path is in the DOM
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const update = () => {
      if (!wrapperRef.current || !pathRef.current || !beeRef.current) return

      const rect      = wrapperRef.current.getBoundingClientRect()
      const vh        = window.innerHeight
      const scrollable = rect.height - vh

      // Sticky mode on desktop (wrapper taller than viewport);
      // fallback to viewport-intersection formula on mobile
      const p = scrollable > vh * 0.3
        ? Math.max(0, Math.min(1, -rect.top / scrollable))
        : Math.max(0, Math.min(1, (vh * 0.72 - rect.top) / (rect.height * 1.2)))
  ...
  return (
    <div className="bee-tl__wrapper" ref={wrapperRef}>
      <section className="bee-tl">
